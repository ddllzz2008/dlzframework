...
@interface DetailModifyViewController (){
    ...
    BusIncome *income;
    BusExpenditure *expend;
}

@end

@implementation DetailModifyViewController
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    if (detailType==0) {
        ...
        [titleView setText:NSLocalizedString(@"detailmodifyviewcontroller_002", nil)];
        
         income = [[IncomeDAL Instance] getIncomeDetail:detailId];
        if (income!=nil) {
            [spendAccount setText:[NSString stringWithFormat:@"%.1f",income.IVALUE]];
            model_categoryID = income.CID;
            model_packageID=income.PID;
            model_choosedate = [income.CREATETIME convertDateFromString:dateformat_07];
            if ([[model_choosedate formatWithCode:dateformat_03] isEqualToString:[[NSDate date] formatWithCode:dateformat_03]]) {
                [dateSelected setText:@"今天"];
            }
            else if ([[model_choosedate formatWithCode:dateformat_03] isEqualToString:[[NSDate dateForDays:-1] formatWithCode:dateformat_03]]){
                [dateSelected setText:@"昨天"];
            }
            else{
                [dateSelected setText:[model_choosedate formatWithCode:dateformat_03]];
            }
            NSPredicate *typePre = [NSPredicate predicateWithFormat:@"CID==%@",income.CID];
            NSArray *typeArray = [typeSource filteredArrayUsingPredicate:typePre];
            if (typeArray!=nil&&typeArray.count>0) {
                [categorySelected setText:((CategoryModel*)[typeArray objectAtIndex:0]).CNAME];
            }
            NSPredicate *packagePre = [NSPredicate predicateWithFormat:@"PID==%@",income.PID];
            NSArray *packageArray = [packageSource filteredArrayUsingPredicate:packagePre];
            if (packageArray!=nil&&packageArray.count>0) {
                [packageSelected setText:((PackageCompare*)[packageArray objectAtIndex:0]).PNAME];
            }
            [markText setText:income.IMARK];
        }
    }else{
        
        [titleView setText:NSLocalizedString(@"detailmodifyviewcontroller_001", nil)];
        
        expend = [[ExpenditureDAL Instance] getExpendDetail:detailId];
        if (expend!=nil) {
            [spendAccount setText:[NSString stringWithFormat:@"%.1f",expend.EVALUE]];
            model_categoryID = expend.CID;
            model_packageID=expend.PID;
            model_choosedate = [expend.CREATETIME convertDateFromString:dateformat_07];
            if ([[model_choosedate formatWithCode:dateformat_03] isEqualToString:[[NSDate date] formatWithCode:dateformat_03]]) {
                [dateSelected setText:@"今天"];
            }
            else if ([[model_choosedate formatWithCode:dateformat_03] isEqualToString:[[NSDate dateForDays:-1] formatWithCode:dateformat_03]]){
            ...
            NSPredicate *typePre = [NSPredicate predicateWithFormat:@"CID==%@",expend.CID];
            NSArray *typeArray = [typeSource filteredArrayUsingPredicate:typePre];
            if (typeArray!=nil&&typeArray.count>0) {
                [categorySelected setText:((CategoryModel*)[typeArray objectAtIndex:0]).CNAME];
            }
            NSPredicate *packagePre = [NSPredicate predicateWithFormat:@"PID==%@",expend.PID];
            NSArray *packageArray = [packageSource filteredArrayUsingPredicate:packagePre];
            if (packageArray!=nil&&packageArray.count>0) {
                [packageSelected setText:((PackageCompare*)[packageArray objectAtIndex:0]).PNAME];
            }
            [markText setText:expend.IMARK];
        }
    }
    /*--------end---------*/
}
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (segmentType.selectedSegmentIndex == 0) {
        if (indexPath.item == typeSource.count - 1) {
            NSInteger typeID = detailType==0?1:0;
