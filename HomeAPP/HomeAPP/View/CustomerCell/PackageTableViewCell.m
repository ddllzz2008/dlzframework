...
@interface PackageTableViewCell(){
    ...
    __strong UILabel *packageLabel;
    __strong UILabel *lastLabel;
...
+(instancetype)cellWithTableView:(UITableView *)tableView indexpath:(NSIndexPath*)indexpath{
    ...
    if (cell==nil) {
        cell = [[PackageTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier indexpath:indexpath];
...
-(id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier indexpath:(NSIndexPath*)indexpath{
    ...
    if (self) {
        ...
        [scrollCell setExclusiveTouch:YES];
        ...
        packageLabel = [UILabel new];
        packageLabel.textAlignment=NSTextAlignmentLeft;
        [packageLabel styleForNormal];
        [pointView addSubview:packageLabel];
...
//            if (i==1) {
//                sub.frame =CGRectMake(0, 0, 35, 60);
//            }else if(i==0){
//                sub.frame =CGRectMake(40, 0, 35, 60);
//            }
//        }
    }
}
...
-(void)setLeftValue:(NSString*)title{
    ...
    CGSize size = [lastLabel getLabelSize:CGSizeMake(80, 30)];
    size=CGSizeMake(size.width+20, 30);
    [lastLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(pointView.mas_right).with.offset(-100);
        make.centerY.equalTo(self.contentView);
        make.size.mas_equalTo(CGSizeMake(size.width, size.width));
    }];
}

-(void)setRightValue:(NSString*)title{
    currentLabel.text=title;
    CGSize size = [currentLabel getLabelSize:CGSizeMake(80, 30)];
    size=CGSizeMake(size.width+20, 30);
    [currentLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(pointView.mas_right).with.offset(-10);
        ...
        if (scrollView.contentOffset.x > 20) {
            ...
            [UIView animateWithDuration:0.25 animations:^{
                ...
                if (self.moveComplete!=nil) {
                    __weak PackageTableViewCell *cell = self;
